...
### RC-F040 — Pre-National Crobat evolution passed target selection but was canceled inside the evolution scene
**Status:** FIXED / CI PENDING / RUNTIME RETEST REQUIRED  
MyBoy runtime on the frozen RC `a1c7fa573ac784ef089bfdf966aa38fc84861212` exposed a real EVO-013 blocker. A Golbat with sufficient friendship began evolving before the National Dex, proving that `GetEvolutionTargetSpecies()` correctly selected Crobat, but `Task_EvolutionScene()` still contained vanilla's second-stage guard that automatically canceled any normal evolution whose target species ID is above Mew while the National Dex is disabled. This also threatened approved direct-use cross-generation item evolutions because they enter the same normal evolution scene. The obsolete normal-scene cancellation has been removed; player B-button cancellation remains intact, and the separate trade-evolution scene is unchanged so link/trade policy is not broadened accidentally. `validate_rc_freeze.py` now rejects any return of the normal animation-time National-Dex blocker. The old RC is invalidated for final acceptance and Crobat/direct-use cross-generation evolution require MyBoy retest on the replacement RC.
